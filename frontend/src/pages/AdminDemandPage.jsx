...
import AdminDemandCard from '../components/AdminDemandCard.jsx'
...
function DesktopDemandTable({ demands, onOpenLogs, onOperate }) {
  return (
    <div className="admin-demand-table-wrap admin-demand-desktop">
      <table className="admin-demand-table">
        <thead>
          <tr><th>需求</th><th>状态</th><th>运营信息</th><th>家长 / 投递</th><th>操作</th></tr>
        </thead>
        <tbody>
          {demands.map((demand) => (
            <tr key={demand.id}>
              <td>
                ...
                <span>{demand.child_grade} · {demand.subject}</span>
              ...
              <td>
                ...
                <span>有效期：{formatDate(demand.expires_at)}</span>
                <span>推荐：{formatDate(demand.featured_at)} 至 {formatDate(demand.featured_until)}</span>
                <span>浏览：{demand.view_count} · 更新：{formatDate(demand.updated_at)}</span>
              </td>
              <td>
                <strong>{demand.parent?.display_name || '—'}</strong>
                <span>{demand.parent?.email || '—'}</span>
                <span>账号：{demand.parent?.status || '—'}</span>
                <span>投递：{demand.application_count}</span>
              </td>
              <td>
...
function AdminDemandPage() {
  ...
  const [logDemand, setLogDemand] = useState(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await getAdminDemands(params)
  ...
  async function operate(demand, action, data) {
    ...
    try {
      if (action === 'list' || action === 'unlist') {
        await updateDemandVisibility(demand.id, data)
      } else if (action === 'feature' || action === 'unfeature') {
        await updateDemandFeature(demand.id, data)
      } else {
        await updateDemandExpiry(demand.id, data)
      }
  ...
  return (
    <section className="page-section admin-demand-page">
      <div className="section-heading admin-demand-heading">
        <div>
          <span className="eyebrow">Admin · Demands</span>
          <h1>需求运营</h1>
          <p>审核公开信息，控制需求上架、推荐与有效期。</p>
        </div>
        <Link className="secondary-button" to="/admin/dashboard">返回控制台</Link>
      </div>
      ...
      {loading ? (
        <LoadingState label="正在加载需求…" />
      ) : !error && !demands.length ? (
        <EmptyState description="请调整筛选条件后重试。" title="没有符合条件的需求" />
      ) : !error && (
        <>
          <DesktopDemandTable demands={demands} onOpenLogs={setLogDemand} onOperate={operate} />
          <div className="admin-demand-mobile-list">
            {demands.map((demand) => (
              <AdminDemandCard
                demand={demand}
                key={demand.id}
                onOpenLogs={setLogDemand}
                onOperate={operate}
              />
            ))}
          </div>
      ...
      {!loading && !error && pagination.total_pages > 1 && (
        <div className="pagination">
          ...
          <span>第 {page} / {pagination.total_pages} 页，共 {pagination.total} 条</span>
          <button disabled={page >= pagination.total_pages} onClick={() => setPage(page + 1)} type="button">下一页</button>
        </div>
      )}
